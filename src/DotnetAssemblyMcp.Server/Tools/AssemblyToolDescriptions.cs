...
internal static class AssemblyToolDescriptions
{
    ...
    // GetMethod.
    internal const string GetMethod_Summary = """Implements the consumer side of the MethodIdentity handoff contract: given a moduleVersionId + metadataToken (typically copied from a dotnet-diagnostics-mcp response), returns the declaring type, method name, signature, IL size and method attributes. The rendered signature surfaces custom modifiers inline (e.g. 'int32 modreq(System.Runtime.InteropServices.InAttribute)' for in / ref readonly, 'modreq(System.Runtime.CompilerServices.IsExternalInit)' for init-only setters, 'modopt(System.Runtime.CompilerServices.IsVolatile)' for volatile types) so consumers can distinguish 'in T' from 'ref T' and spot init / volatile semantics without decompiling. For methods carrying the PinvokeImpl bit, MethodSummary.PInvoke is populated with the decoded DllImport binding (module name, entry point, char set, calling convention, ExactSpelling / SetLastError / PreserveSig). For generic methods, MethodSummary.GenericParameters lists each type parameter with its variance, special constraints (class / struct / new()) and any base-type / interface constraints. See docs/handoff-contract.md for the full semantics and error kinds.""";
