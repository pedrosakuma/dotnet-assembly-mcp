...
(not an agent) want to navigate an assembly — every MCP tool has a matching subcommand, rendered
as human-readable text by default (or `--json` for the full MCP envelope).
...
Full walkthrough, subcommand table, composed commands (`explain-type`, `explain-method`,
`callgraph`, `diff-assemblies`), global options, and exit codes: **[`docs/cli-usage.md`](./docs/cli-usage.md)**.
...
[dotnet-diagnostics-mcp]                    [dotnet-assembly-mcp]
   ──────────────────────                     ──────────────────────
   inspect_process(view="list")                load_assembly
   collect_sample(kind="cpu")          ──┐  ┌─→ get_method
   collect_events(kind="exceptions")      │  │   get_method_il (format='scan')
                                          │  │   decompile_method
                                          ▼  │   find_callers
                                   (MethodIdentity)
```
